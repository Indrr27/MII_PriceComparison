# scrape_and_compare_afcgrocery.py
"""
AFC Grocery scraper and comparison tool with detailed match display.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('afc_grocery_scraping.log'),
...
def setup_afc_store(db: Session) -> Store:
    """Create or get the AFC Grocery store record"""
    store = StoreCRUD.get_store_by_name(db, "AFC Grocery")
    
    if not store:
        store_data = {
            "name": "AFC Grocery",
            "website": "https://afcgrocery.com",
            "location": "Toronto, Ontario",
            "store_type": "Asian/Indian Grocery",
            "is_primary": False
        }
        store = StoreCRUD.create_store(db, store_data)
        logger.info("Created AFC Grocery store record")
    else:
        logger.info("Found existing AFC Grocery store record")
...
def validate_and_clean_scraped_data(products: list) -> list:
    ...
    for product in products:
        ...
        # Note: AFC includes size in product name, no separate size field needed
        cleaned_product = {
            'name': cleaned_name,
            'price': price,
...
def save_afc_products(db: Session, store: Store, products: list):
    """Save scraped AFC products to database with validation"""
...
def display_detailed_matches(matches, quality_report=None):
    ...
    print("\n" + "="*120)
    print("🎯 DETAILED MATCH RESULTS - AFC GROCERY")
    print("="*120)
...
def validate_store_data_quality():
    ...
    try:
        ...
        afc = StoreCRUD.get_store_by_name(db, "AFC Grocery")
        
        if not made_in_india or not afc:
            return False, "Missing store data"
        
        mii_products = ProductCRUD.get_products_by_store(db, made_in_india.id)
        afc_products = ProductCRUD.get_products_by_store(db, afc.id)
        
        print(f"\n🔍 DATA QUALITY VALIDATION")
        print(f"{'='*50}")
        print(f"Made in India: {len(mii_products)} products")
        print(f"AFC Grocery: {len(afc_products)} products")
        
        if len(mii_products) < 10:
            return False, "Not enough Made in India products for meaningful comparison"
        
        if len(afc_products) < 50:
            return False, "Not enough AFC products for meaningful comparison"
        
        # Check price data availability
        mii_with_prices = sum(1 for p in mii_products if PriceCRUD.get_latest_price(db, p.id))
        afc_with_prices = sum(1 for p in afc_products if PriceCRUD.get_latest_price(db, p.id))
        
        print(f"With prices - MII: {mii_with_prices}/{len(mii_products)}, AFC: {afc_with_prices}/{len(afc_products)}")
        
        if mii_with_prices / len(mii_products) < 0.8:
            return False, "Too many Made in India products missing prices"
        
        if afc_with_prices / len(afc_products) < 0.8:
            return False, "Too many AFC products missing prices"
...
def run_ai_matching_with_display():
    ...
    print("\n🎯 AI MATCHING WITH DETAILED DISPLAY - AFC GROCERY")
    print("="*60)
...
def main():
    ...
    print("🛒 ENHANCED AFC GROCERY SCRAPER & COMPARISON")
    ...
    try:
        ...
        # Setup AFC store
        afc_store = setup_afc_store(db)
        existing_afc_products = ProductCRUD.get_products_by_store(db, afc_store.id)
        print(f"ℹ️ Found {len(existing_afc_products)} existing AFC Grocery products")
        
        # Enhanced menu
        print("\n🎯 What would you like to do?")
        print("1. Scrape AFC Grocery products (10-15 minutes)")
        ...
        if choice in ['1', '3']:
            # Scrape AFC
            logger.info("Starting AFC Grocery scraping...")
            print("🛒 Starting AFC Grocery scraping...")
            print("⏱️ Expected time: 10-15 minutes")
            print("📝 Note: AFC has significant overlap with Made in India Grocery")
            ...
            scraper = AFCGroceryScraper(headless=True)
            products = scraper.scrape_with_error_handling()
            
            if not products:
                logger.error("❌ No products were scraped from AFC Grocery!")
                return
            
            logger.info(f"✅ Successfully scraped {len(products)} products from AFC")
            
            # Save to database
            logger.info("💾 Saving products to database...")
            saved_count, updated_count = save_afc_products(db, afc_store, products)
