# scrape_and_compare_nofrills.py 
"""
Enhanced No Frills scraper and comparison tool with detailed match display.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('nofrills_scraping.log'),
...
def setup_nofrills_store(db: Session) -> Store:
    """Create or get the No Frills store record"""
    store = StoreCRUD.get_store_by_name(db, "No Frills")
    ...
    if not store:
        ...
        logger.info("Created No Frills store record")
    else:
        logger.info("Found existing No Frills store record")
...
def validate_and_clean_scraped_data(products: list) -> list:
    ...
    logger.info(f"   ⚠️ Missing names: {issues_found['missing_name']}")
    logger.info(f"   ⚠️ Missing/invalid prices: {issues_found['missing_price']}")
    logger.info(f"   ⚠️ Invalid price format: {issues_found['invalid_price']}")
    
    return cleaned_products

def save_nofrills_products(db: Session, store: Store, products: list):
    """Save scraped No Frills products to database with validation"""
...
def display_detailed_matches(matches, quality_report=None):
    """Display detailed match information in console (same as Superstore)"""
    if not matches:
...
def validate_store_data_quality():
    ...
    try:
        ...
        nofrills = StoreCRUD.get_store_by_name(db, "No Frills")
        
        if not made_in_india or not nofrills:
            return False, "Missing store data"
        
        mii_products = ProductCRUD.get_products_by_store(db, made_in_india.id)
        nf_products = ProductCRUD.get_products_by_store(db, nofrills.id)
        
        print(f"\n🔍 DATA QUALITY VALIDATION")
        print(f"{'='*50}")
        print(f"Made in India: {len(mii_products)} products")
        print(f"No Frills: {len(nf_products)} products")
        
        if len(mii_products) < 10:
            return False, "Not enough Made in India products for meaningful comparison"
        
        if len(nf_products) < 50:
            return False, "Not enough No Frills products for meaningful comparison"
        
        # Check price data availability
        mii_with_prices = sum(1 for p in mii_products if PriceCRUD.get_latest_price(db, p.id))
        nf_with_prices = sum(1 for p in nf_products if PriceCRUD.get_latest_price(db, p.id))
        
        print(f"With prices - MII: {mii_with_prices}/{len(mii_products)}, NF: {nf_with_prices}/{len(nf_products)}")
        
        if mii_with_prices / len(mii_products) < 0.8:
            return False, "Too many Made in India products missing prices"
        
        if nf_with_prices / len(nf_products) < 0.8:
            return False, "Too many No Frills products missing prices"
...
def run_ai_matching_with_display():
    ...
    print("\n🎯 AI MATCHING WITH DETAILED DISPLAY")
    print("="*60)
    ...
    if show_reports == 'y':
        ...
        matcher.generate_price_report("Made in India Grocery", min_confidence, use_normalized=use_normalization, competitor_store="No Frills")
...
def main():
    ...
    print("🛒 ENHANCED NO FRILLS SCRAPER & COMPARISON")
    ...
    try:
        ...
        # Setup No Frills store
        nofrills_store = setup_nofrills_store(db)
        existing_nofrills_products = ProductCRUD.get_products_by_store(db, nofrills_store.id)
        print(f"ℹ️ Found {len(existing_nofrills_products)} existing No Frills products")
        
        # Enhanced menu
        print("\n🎯 What would you like to do?")
        print("1. Scrape No Frills products (15-20 minutes)")
        ...
        if choice in ['1', '3']:
            # Scrape No Frills
            logger.info("Starting No Frills scraping...")
            print("🏪 Starting No Frills scraping...")
            ...
            scraper = NoFrillsScraper(headless=True)
            products = scraper.scrape_with_error_handling()
            
            if not products:
                logger.error("⚠️ No products were scraped from No Frills!")
                return
            
            logger.info(f"✅ Successfully scraped {len(products)} products from No Frills")
            
            # Save to database
            logger.info("💾 Saving products to database...")
            saved_count, updated_count = save_nofrills_products(db, nofrills_store, products)
        ...
        if choice == '4':
            ...
            is_valid, message = validate_store_data_quality()
            print(f"\n📊 Validation Result: {'✅ PASSED' if is_valid else '⚠️ FAILED'}")
        ...
        if choice == '6':
            ...
            matcher = DatabaseMatcher()
            matcher.generate_match_report("Made in India Grocery")
            matcher.generate_price_report("Made in India Grocery", min_confidence, use_normalized=use_normalized, competitor_store="No Frills")
