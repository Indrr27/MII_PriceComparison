# scrape_and_compare_superstore.py
"""
Enhanced scraper and comparison tool with detailed match display.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('superstore_scraping.log'),
...
def setup_superstore_store(db: Session) -> Store:
    """Create or get the Superstore store record"""
    store = StoreCRUD.get_store_by_name(db, "Real Canadian Superstore")
    ...
    if not store:
        ...
        logger.info("Created Real Canadian Superstore store record")
    else:
        logger.info("Found existing Real Canadian Superstore store record")
...
def save_superstore_products(db: Session, store: Store, products: list):
    """Save scraped Superstore products to database with validation"""
...
def validate_store_data_quality():
    ...
    try:
        ...
        superstore = StoreCRUD.get_store_by_name(db, "Real Canadian Superstore")
        
        if not made_in_india or not superstore:
            return False, "Missing store data"
        
        mii_products = ProductCRUD.get_products_by_store(db, made_in_india.id)
        ss_products = ProductCRUD.get_products_by_store(db, superstore.id)
        
        print(f"\n🔍 DATA QUALITY VALIDATION")
        print(f"{'='*50}")
        print(f"Made in India: {len(mii_products)} products")
        print(f"Superstore: {len(ss_products)} products")
        
        if len(mii_products) < 10:
            return False, "Not enough Made in India products for meaningful comparison"
        
        if len(ss_products) < 50:
            return False, "Not enough Superstore products for meaningful comparison"
        
        # Check price data availability
        mii_with_prices = sum(1 for p in mii_products if PriceCRUD.get_latest_price(db, p.id))
        ss_with_prices = sum(1 for p in ss_products if PriceCRUD.get_latest_price(db, p.id))
        
        print(f"With prices - MII: {mii_with_prices}/{len(mii_products)}, SS: {ss_with_prices}/{len(ss_products)}")
        
        if mii_with_prices / len(mii_products) < 0.8:
            return False, "Too many Made in India products missing prices"
        
        if ss_with_prices / len(ss_products) < 0.8:
            return False, "Too many Superstore products missing prices"
...
def run_ai_matching_with_display():
    ...
    if show_reports == 'y':
        ...
        matcher.generate_price_report("Made in India Grocery", min_confidence, use_normalized=use_normalization, competitor_store="Real Canadian Superstore")
...
def main():
    ...
    try:
        ...
        if not made_in_india:
            print("❌ Made in India Grocery not found in database!")
        ...
        # Setup Superstore store
        superstore_store = setup_superstore_store(db)
        existing_superstore_products = ProductCRUD.get_products_by_store(db, superstore_store.id)
        print(f"ℹ️ Found {len(existing_superstore_products)} existing Superstore products")
        
        # Enhanced menu
        print("\n🎯 What would you like to do?")
        print("1. Scrape Superstore products (15-20 minutes)")
        ...
        if choice in ['1', '3']:
            # Scrape Superstore
            logger.info("Starting Superstore scraping...")
            print("🕐 Starting Superstore scraping...")
            ...
            scraper = SuperstoreScraper(headless=True)
            products = scraper.scrape_with_error_handling()
            
            if not products:
                logger.error("❌ No products were scraped from Superstore!")
                return
            
            logger.info(f"✅ Successfully scraped {len(products)} products from Superstore")
            
            # Save to database
            logger.info("💾 Saving products to database...")
            saved_count, updated_count = save_superstore_products(db, superstore_store, products)
        ...
        if choice == '6':
            ...
            matcher = DatabaseMatcher()
            matcher.generate_match_report("Made in India Grocery")
            matcher.generate_price_report("Made in India Grocery", min_confidence, use_normalized=use_normalized, competitor_store="Real Canadian Superstore")
